# /packages/intranet-reporting/www/finance-subprojects-documents.tcl
#
...
ad_page_contract {
    Shows the project hierarchy with associated financial items.
...
set levels {1 "Customer Only" 2 "Customer + Main Project" 3 "Customer + Main + Subjproject" 4 "All Details"} 
...
if {0 != $main_project_id && "" != $main_project_id} {
    lappend criteria "parent.project_id = :main_project_id"
}
...
if {0 != $project_id && "" != $project_id} {
    lappend criteria "child.project_id in (
	select
		p.project_id
	from
		im_projects p,
...
if {[llength $project_status_ids] > 0} {
    lappend criteria "child.project_status_id in ([join $project_status_ids ","])"
} 
...
set inner_sql "
	select
		...
		-- Project fields of subproject
		child.*,
		child.end_date::date as project_end_date,
		im_category_from_id(child.project_status_id) as project_status,
		im_category_from_id(child.project_type_id) as project_type,
		trunc(child.percent_completed::numeric, 2) as percent_completed_formatted,
		'<a href=/intranet/users/view?user_id=' || child.project_lead_id || '>' || 
			im_name_from_user_id(child.project_lead_id) || '</a>' as project_lead_link,
		to_char(child.project_budget, :cur_format) || ' ' || child.project_budget_currency as project_budget_formatted,
		to_char(child.end_date, :date_format) as end_date_formatted,
		to_char(child.start_date, :date_format) as start_date_formatted,
		'<a href=/intranet/users/view?user_id=' || child.company_contact_id || '>' || 
			im_name_from_user_id(child.company_contact_id) || '</a>' as company_contact_link
		...
		-- Objects Fields
		o.creation_user as sales_rep_id,
		im_name_from_user_id(o.creation_user) as sales_rep_name,
	...
	from
		...
		LEFT OUTER JOIN im_costs c ON (c.cost_id = r.object_id_two OR c.project_id = child.project_id)
		...
		LEFT OUTER JOIN acs_objects o ON (c.cost_id = o.object_id)
	where
		parent.parent_id is null and
		parent.project_status_id not in ([im_project_status_deleted]) and
		parent.end_date >= to_date(:start_date, 'YYYY-MM-DD') and
		parent.end_date < to_date(:end_date, 'YYYY-MM-DD') and
...
for {set i 1} {$i <= $max_col} {incr i} {
    ...
    lappend parent_project_footer $cont
}

set parent_project_footer [concat $parent_project_footer [list \
	"" \
...
set report_def [list \
    ...
    header {
	"\#colspan=15 <a href=$this_url&customer_id=$customer_id&level_of_detail=3
	...
	<b><a href=$company_url$customer_id>$customer_name</a></b>"
	    ...
	    header {
		"HMain:"
		"\#colspan=14 <a href=$this_url&parent_project_id=$parent_project_id&level_of_detail=4 
		target=_blank><img src=/intranet/images/plus_9.gif width=9 height=9 border=0></a> 
		<b><a href=$project_url$parent_project_id>$parent_project_name</a></b>"
	    } \
	    content [list \
	            group_by project_id \
		    header {
			"HSub:"
			"$tree_sortkey_pretty"
			"\#colspan=13 $tree_sortkey_spacers<a href=$this_url&project_id=$project_id&level_of_detail=4 
			target=_blank><img src=/intranet/images/plus_9.gif width=9 height=9 border=0></a> 
            ...
            footer $parent_project_footer \
    ] \
    footer $project_customer_footer \
]

# Global header/footer
set header0 [list "Cust" "Main Project" "Sub Project" "Name"]
...
#
set invoice_subtotal_counter [list pretty_name "Invoice Amount" var invoice_subtotal reset \$customer_id expr "\$invoice_amount+0"]
set delnote_subtotal_counter [list pretty_name "Delnote Amount" var delnote_subtotal reset \$customer_id expr "\$delnote_amount+0"]
set quote_subtotal_counter [list pretty_name "Quote Amount" var quote_subtotal reset \$customer_id expr "\$quote_amount+0"]
set bill_subtotal_counter [list pretty_name "Bill Amount" var bill_subtotal reset \$customer_id expr "\$bill_amount+0"]
set po_subtotal_counter [list pretty_name "Po Amount" var po_subtotal reset \$customer_id expr "\$po_amount+0"]
set expense_subtotal_counter [list pretty_name "Expense Amount" var expense_subtotal reset \$customer_id expr "\$expense_amount+0"]
set timesheet_subtotal_counter [list pretty_name "Timesheet Amount" var timesheet_subtotal reset \$customer_id expr "\$timesheet_amount+0"]
	...
	<td>
		...
		<tr>
		  ...
		  <td class=form-widget>
	        ...
	        <tr>
	          <td class=form-label>Main Project</td>
	          <td class=form-widget>
	            [im_project_select main_project_id $main_project_id]
	          </td>
	        </tr>
	        <tr>
...
ns_log Notice "intranet-reporting-finance/finance-projects-documents: sql=\n$sql"
...
db_foreach sql $sql {
    ...
    set tree_sortkey_pretty [im_reporting_tree_sortkey_pretty $tree_sortkey]
    set tree_sortkey_spacers [im_reporting_tree_sortkey_spacers $tree_sortkey]
