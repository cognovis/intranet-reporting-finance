...
set inner_sql "
	...
	from
		acs_objects o,
		(select	p.project_id as project_project_id,
			c.*
		from	(
				select	c.project_id,
					c.cost_id
...
db_foreach sql $sql {
	...
	set gross_profit_subsubtotal [expr $invoice_subsubtotal - $po_subsubtotal - $expense_subsubtotal]
	set wip_subsubtotal [expr $timesheet_subsubtotal + $po_subsubtotal + $expense_subsubtotal - $invoice_subsubtotal]
